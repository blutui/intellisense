...
- Documented `canopy.blocks`'s full signature (`handle, blocksOrOptions, options`) — seeding a zone with an explicit template list, `locked` (structure can't be changed in the editor), and `limit` (max blocks a zone accepts) — verified against `Tenant\Twig\Modules\CanopyModule` in the Blutui backend, and added `canopy.blocks.seed`/`canopy.blocks.locked`/`canopy.blocks.limit` snippets
