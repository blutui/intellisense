...
const node_fetch_1 = __webpack_require__(1);
const vscode = __webpack_require__(7);
...
function activate(context) {
    ...
    const collectionAuto = vscode.languages.registerCompletionItemProvider('html', {
    ...
    const collectionAutoI = vscode.languages.registerCompletionItemProvider('html', {
        provideCompletionItems(document, position) {
            const linePrefix = document.lineAt(position).text.substr(0, position.character);
    ...
    const formAuto = vscode.languages.registerCompletionItemProvider('html', {
        provideCompletionItems(document, position) {
            const linePrefix = document.lineAt(position).text.substr(0, position.character);
    ...
    const disposable = () => __awaiter(this, void 0, void 0, function* () {
        console.log("Calling Api");
        if (vscode.workspace.workspaceFolders !== undefined) {
            let wf = vscode.workspace.workspaceFolders[0].uri.path;
            let uri = vscode_1.Uri.file(wf + '/courier.json');
            vscode.workspace.openTextDocument(uri).then((document) => __awaiter(this, void 0, void 0, function* () {
                ...
                const response = yield node_fetch_1.default(urlL, {
                    method: 'GET',
                    headers: {
                ...
                collectionHandle = myJson.map((a) => a.handle);
                collectionID = myJson.map((a) => a.id);
                const collect = [];
                const collectOp = [];
                for (const i of collectionID) {
                    let urlID = urlL + "/" + i;
                    const r = yield node_fetch_1.default(urlID, {
                        method: 'GET',
                        headers: {
                            Authorization: website.token
                        }
                    });
                    const col = yield r.json();
                    collectionVar = yield col.fields.map((a) => a.name);
                    let collection = {
                        handle: col.handle,
                ...
                let urlF = url + "/forms";
                const g = yield node_fetch_1.default(urlF, {
                    method: 'GET',
                    headers: {
                        Authorization: website.token
                    }
                });
                const j = yield g.json();
                let form = j.map((a) => a.handle);
                let formL = j.map((a) => a.template);
                let formB = j.map((a) => a.fields.map((b) => b.name));
                const f = [];
                ...
                FormObjects = f;
                const formOp = [];
                formHandle = FormObjects.map((a) => a.handle);
                f.forEach(element => {
                    element.fields.forEach((el) => {
...
/* harmony import */ var url__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(4);
